...
import type { FeedbackWithRelations, TableColumn, FeedbackCommentWithUser } from '@/types';
...
export default function FeedbacksPage() {
  ...
  const addCommentMutation = useMutation({
    mutationFn: (data: { feedbackId: string; content: string }) => 
      feedbackService.addComment(data.feedbackId, data.content, 'manager'),
    onSuccess: async () => {
      if (selectedFeedback) {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: (data: { feedbackId: string; status: 'pending' | 'in_progress' | 'resolved' | 'rejected' }) => 
      feedbackService.updateFeedbackStatus(data.feedbackId, data.status),
    onSuccess: async () => {
      if (selectedFeedback) {
        const updated = await feedbackService.getFeedbackById(selectedFeedback.id);
        setSelectedFeedback(updated);
  ...
  const sortedData = React.useMemo(() => {
    ...
    sorted.sort((a, b) => {
      const aValue = a[sortField as keyof FeedbackWithRelations];
      const bValue = b[sortField as keyof FeedbackWithRelations];
  ...
  const handleRowClick = React.useCallback(async (feedback: FeedbackWithRelations) => {
    setIsModalOpen(true);
  ...
  const handleUpdateStatus = React.useCallback((status: 'pending' | 'in_progress' | 'resolved' | 'rejected') => {
    if (!selectedFeedback) return;
    updateStatusMutation.mutate({
      feedbackId: selectedFeedback.id,
      status,
    });
  }, [selectedFeedback, updateStatusMutation]);

  const columns: TableColumn<FeedbackWithRelations>[] = [
    {
      key: 'title',
      label: 'Tiêu đề',
      sortable: true,
    ...
    },
    {
      key: 'priority',
      label: 'Ưu tiên',
      render: (value) => <StatusBadge status={String(value)} type="priority" />,
    },
    {
      key: 'status',
      label: 'Trạng thái',
      sortable: true,
      render: (value) => <StatusBadge status={String(value)} type="feedback" />,
    },
    {
      key: 'created_at',
      label: 'Ngày tạo',
  ...
  if (isLoading) {
    return <Loading text="Đang tải danh sách phản ánh..." />;
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Danh sách tất cả phản ánh từ cư dân
        </p>
      </div>
          ...
          <div className="space-y-6">
            <div className="space-y-4 pb-4 border-b">
              ...
              {selectedFeedback.assigned_to && (
                <div>
                  ...
                  <p className="mt-1">{selectedFeedback.assignee?.full_name || '-'}</p>
                </div>
              )}

              {selectedFeedback.resolution_notes && (
                <div>
                  <label className="text-sm font-medium text-gray-500">Ghi chú khắc phục</label>
                  <p className="mt-1 whitespace-pre-wrap">{selectedFeedback.resolution_notes}</p>
                </div>
              )}
            </div>
            ...
            {selectedFeedback.status !== 'resolved' && selectedFeedback.status !== 'rejected' && (
              <div className="pb-4 border-b">
                <label className="text-sm font-medium text-gray-500 mb-2 block">Cập nhật trạng thái</label>
