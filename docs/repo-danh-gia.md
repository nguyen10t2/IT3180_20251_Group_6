...
## Tổng quan
- Backend chạy trên Bun với Elysia, Drizzle ORM và PostgreSQL (`src/server.ts` là entry point).
...
- Chưa có tài liệu hướng dẫn thiết lập, biến môi trường, migrate DB; README mặc định từ template.
- Không có kiểm thử tự động; script `test` đang trả về lỗi mặc định.
- Các route xác thực bị comment; API hoạt động thực tế chỉ có `/` và `/create`.
- `createUser` lưu mật khẩu dạng plain text, chưa có hashing → rủi ro bảo mật nghiêm trọng.
- Chưa kiểm tra/validate biến môi trường (ví dụ `DATABASE_URL` được truy cập trực tiếp, có thể gây crash khi thiếu).
